...
import ActivityApi from '../../api/single/activity'
...
import BatchFetchColumn from '../../api/batch/column'
...
class BatchFetchAuthorActivity extends Base {
  async fetch(urlToken: string) {
    ...
    this.log(`用户${name}(${urlToken})最后一次活跃于${dayjs.unix(endAt).format(DATE_FORMAT.Const_Display_By_Second)}`)
    ...
    for (let checkAt = startAt; checkAt <= endAt;) {
      ...
      if (hasActivityAfterAt) {
        this.log(
          `经检查, 用户${name}(${urlToken})在${dayjs
            .unix(checkAt)
            .format(DATE_FORMAT.Const_Display_By_Second)}前有活动记录`,
        )
        this.log(`检查完毕`)
        startAt = dayjs.unix(checkAt).startOf(DATE_FORMAT.Const_Unit_Month).unix()
        break
      } else {
        this.log(
          `经检查, 用户${name}(${urlToken})在${dayjs
        ...
        let newCheckAt = dayjs.unix(checkAt).add(1, DATE_FORMAT.Const_Unit_Month).unix()
        checkAt = newCheckAt
      }
    ...
    this.log(
      `用户活动时间范围为${dayjs.unix(startAt).format(DATE_FORMAT.Const_Display_By_Second)} ~ ${dayjs
        .unix(endAt)
    ...
    for (let fetchAt = startAt; startAt <= fetchAt && fetchAt <= endAt;) {
      ...
      let fetchEndAt = dayjs.unix(fetchAt).endOf(DATE_FORMAT.Const_Unit_Month).unix()
  ...
  private async fetchActivityInRange(urlToken: string, startAt: number, endAt: number) {
    let rangeString = `${dayjs.unix(startAt).format(DATE_FORMAT.Const_Display_By_Day)} ~ ${dayjs
      .unix(endAt)
    ...
    for (let fetchAt = endAt; startAt <= fetchAt && fetchAt <= endAt;) {
      let asyncTaskFunc = async () => {
        this.log(`[${rangeString}]抓取${dayjs.unix(fetchAt).format(DATE_FORMAT.Const_Display_By_Second)}的记录`)
